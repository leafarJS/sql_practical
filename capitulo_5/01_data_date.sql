...
insert into alumnos(numcontrol,nombre,apellidopaterno,apellidomaterno,fechaingreso,fechanacimiento,telefono,carrera,promedio)
...
'1234564788','Medicina',95.2),(1023,'Juan','Sanchez','Perez','2016-05-08','1992-02-18',
'1234564343','Mercadotecnia',72),(1024,'Juan','Vazquez','Galvez','2018-02-08','1991-05-12',
'1234564348','Ingenieria en Sistemas',85.2),(1002,'Carlos','Andrade','Perez','2017-03-15','1989-07-15',
'1234564349','Fisico Matematico',75),(1025,'Maria','Perez','Perez','2018-02-02','1995-08-17',
'1234564322','Psicologia',55.8),(1026,'Guadalupe','Gavilan','Gonzalez','2018-07-25','1995-06-18',
'1234564335','Matematicas puras',89.2);

select * from alumnos;

select now(); # hora exacta
select curdate(); # solo fecha
select curdate() as 'fecha actual'; # solo fecha con alias
select current_date(); # solo fecha
select current_time(); # solo la hora
select current_timestamp(); # fecha y hora 

select year(now()); # solo el año
select month(now()); # solo el dia
select day(now()); # solo el dia
select dayname(now()); # nombre del dia en ingles
select dayofweek(now()); # dia de la semana
select dayofyear(now()); # dia del año
select time(now()); # hora de la fecha actual
select last_day(now()); # ultimo dia del mes

select concat_ws(' ', nombre,apellidopaterno,apellidomaterno) as 'nombre',
fechaingreso, promedio
from alumnos
where (select year(fechaingreso)) = '2021'
and (select month(fechaingreso)) = '07';
